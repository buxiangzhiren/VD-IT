...
class DownEncoderBlock2D(nn.Module):
    ...
    def forward(self, hidden_states: torch.FloatTensor, counting: int=0, scale: float = 1.0) -> torch.FloatTensor:
        ret_ft = []
        for resnet in self.resnets:
            hidden_states = resnet(hidden_states, temb=None, scale=scale)
            counting = counting + 1
            if counting == 3 or counting ==6:
                ret_ft.append(hidden_states)

        if self.downsamplers is not None:
            for downsampler in self.downsamplers:
                hidden_states = downsampler(hidden_states, scale)

        return hidden_states, counting, ret_ft
...
class UpDecoderBlock2D(nn.Module):
    ...
    def forward(
        self, hidden_states: torch.FloatTensor, counting: int = 0, temb: Optional[torch.FloatTensor] = None, scale: float = 1.0
    ) -> torch.FloatTensor:
        ret_ft = []
        for resnet in self.resnets:
            hidden_states = resnet(hidden_states, temb=temb, scale=scale)
            counting += 1
            if counting == 6 or counting ==9:
                ret_ft.append(hidden_states)
