...
class VideoFolderDataset(Dataset):
    ...
    def get_frame_buckets(self, vr):
        resize = T.transforms.Resize((self.width, self.height), antialias=True)
    ...
    def __getitem__(self, index):
        ...
        prompt_ids = self.get_prompt_ids(prompt)[0]

        return {"pixel_values": normalize_input(video[0]), "prompt_ids": prompt_ids, "text_prompt": prompt, 'dataset': self.__getname__(), "opixel_values": video[0]}
