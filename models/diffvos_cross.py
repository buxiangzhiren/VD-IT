...
class VDIT(nn.Module):
    """ This is the VDIT module that performs referring video object detection """
    def __init__(self, args, backbone, transformer, num_classes, num_queries, num_feature_levels,
        ...
        Parameters:
            ...
            num_queries: number of object queries, ie detection slot. For ytvos, we recommend 5 queries for each frame.
        ...
        self.query_tensors = nn.Parameter(torch.randn(self.num_queries, hidden_dim))
        # Build Text Encoder
        ...
        self.sentence_proj.requires_grad_(False)
    ...
    def forward(self, samples: NestedTensor, captions, targets):
        ...
        features, visual_pos = self.backbone(samples, captions)
        b = len(captions)
        ...
        query_tokens = self.query_tensors.unsqueeze(dim=1).repeat_interleave(b,dim=1)

        query_tokens = self.fusion_module(visual=query_tokens.unsqueeze(1).unsqueeze(1),
        ...
        # ***** Spectrum-guided Cross-modal Fusion ***** Not used
        srcs = []
        masks = []
        poses = []
        multi_scale_level = 3
        ...
        text_embed = repeat(query_tokens, 'q b c -> b t q c', t=t)
...
def build(args):
    ...
    if 'itcross_video_swin' in args.backbone:
        from models.itcross_video_swin_transformer import build_video_swin_backbone
