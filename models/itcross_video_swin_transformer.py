...
import sys
sys.path.append('./Text-To-Video-Finetuning')
from train_itcross_pn import FeatureExtractor
...
class BackboneBase(nn.Module):
    def __init__(self, backbone: nn.Module, strides=[4, 8, 16, 32], num_channels=[96, 192, 384, 768]):
        ...
        self.diff_ftex = FeatureExtractor(num_channels=self.num_channels)
        self.body = backbone
...
class Joiner(nn.Sequential):
    ...
    def forward(self, tensor_list: NestedTensor, captions: str):
        ...
        xs = self[0](tensor_list, num_frames=t, captions=captions)
